...
task playerTabbedLoop[1000]() {
	...
	{
		playerCount++;
		if(playerCount > 20) break; // Process max 20 players per tick to prevent lag
...
task ServerHeartbeat[1000]() {
    static reportChecked;
    if(++AdminWarning == 15) {
		for(new z = reportChecked; z < MAX_REPORTS && z < reportChecked + 10; z++)
		{
		...
		reportChecked += 10;
		if(reportChecked >= MAX_REPORTS) reportChecked = 0;
		AdminWarning = 0;
	}
	...
	{
		playerCount++;
		if(playerCount > 30) break; // Process max 30 players per tick
		
		if(playerTabbed[i] == 0) {
			// Optimized paycheck calculation
			new payIncrease = (PlayerInfo[i][pLevel] / 2) + 1;
			if(payIncrease > 11) payIncrease = 11;
			PlayerInfo[i][pPayCheck] += payIncrease;
