...
vehicleCountCheck(playerid) {
	new iCount = GetPlayerVehicleCount(playerid);
	if(iCount >= MAX_PLAYERVEHICLES) return 0;
	return 1;
}
...
{
	new currentCars = GetPlayerVehicleCount(playerid);
	new maxSlots = MAX_PLAYERVEHICLES;
	
	return (currentCars >= maxSlots) ? maxSlots : currentCars;
}
	...
	switch(dialogid)
	{
		...
		case DIALOG_CGAMESRAISEPOKER:
		{
			...
			if(GetPVarInt(playerid, "pkrCurrentBet")+GetPVarInt(playerid, "pkrChips") > PokerTable[tableid][pkrActiveBet]+PokerTable[tableid][pkrBlind]/2) {
				...
				new szString[128];
				format(szString, sizeof(szString), "{FFFFFF}Ban co muon theo bai? ($%d-$%d):", PokerTable[tableid][pkrActiveBet]+PokerTable[tableid][pkrBlind]/2, GetPVarInt(playerid, "pkrCurrentBet")+GetPVarInt(playerid, "pkrChips"));
				...
				new szString[128];
				format(szString, sizeof(szString), "{FFFFFF}Ban co muon theo het so tien? (All-In):");
		...
		case DIALOG_CGAMESSELECTPOKER:
		{
			...
			for(new i = 0; i < MAX_POKERTABLES; i++) {
				...
				format(szString, sizeof(szString), "%sB�n Poker %d (%s) - %d nguoi\n", szString, i, szPlaced, PokerTable[i][pkrPlayers]);
			}
...
Group_ReturnType(iGroupType) {
	...
	switch(iGroupType) {
		...
		case 9: szResult = "URL";
		case 10: szResult = "Trucker Delivery";
	}
	return szResult;
}
 	...
 	if(response_code == 200)
 	{
		if(IsPlayerInAnyVehicle(index))
		{
	 	    foreach(new i: Player)
			{
		  	...
		  	strcpy(stationidv[GetPlayerVehicleID(index)], data, 64);
		  	new string[53];
					...
					if(GetPVarInt(playerid, "pkrActiveHand")) {
						if(playerid != pid) {
							if(PokerTable[tableid][pkrActive] == 4 && PokerTable[tableid][pkrDelay] <= 19 && GetPVarInt(playerid, "pkrHide") != 1) {
								strcpy(tmpString, DeckTextdrw[GetPVarInt(playerid, "pkrCard1")+1], sizeof(tmpString));
								PlayerTextDrawSetString(pid, PlayerPokerUI[pid][2+tmp], tmpString);
								strcpy(tmpString, DeckTextdrw[GetPVarInt(playerid, "pkrCard2")+1], sizeof(tmpString));
			...
			if(PokerTable[tableid][pkrActive] < 3) {
				...
				if(PokerTable[tableid][pkrActive] == 4 && PokerTable[tableid][pkrDelay] == 19) {
					if(GetPVarInt(playerid, "pkrActiveHand") && GetPVarInt(playerid, "pkrHide") != 1) {
						...
						strcpy(tmpString, resultString, sizeof(tmpString));
...
stock SendAudioURLToRange(const url[], Float:x, Float:y, Float:z, Float:range)
{
    audiourlid = CreateDynamicSphere(x, y, z, range);
	strcpy(audiourlurl, url, sizeof(audiourlurl));
	audiourlparams[0] = x;
	audiourlparams[1] = y;
	audiourlparams[2] = z;
...
stock PaintballSetupArena(playerid)
{
	...
	strcpy(password, PaintBallArena[a][pbPassword], sizeof(password));
...
stock StripUnderscore(const string[]) // Doesn't remove underscore from original string any more
{
	new iPos, newstring[128];
	strcpy(newstring, string, sizeof(newstring));
...
stock GetJobName(job)
{
	...
	switch(job)
	{
		...
		case 21: name = "Pizza Delivery";
		case 22: name = "Trucker Delivery";
		...
		if(vehicleSpawnCountCheck(playerid)) {
			if(PlayerVehicleInfo[playerid][v][pvModelId] >= 400) {
				if(PlayerVehicleInfo[playerid][v][pvSpawned] && !PlayerVehicleInfo[playerid][v][pvDisabled] && !PlayerVehicleInfo[playerid][v][pvImpounded]) {
					...
					ChangeVehicleColours(carcreated, PlayerVehicleInfo[playerid][v][pvColor1], PlayerVehicleInfo[playerid][v][pvColor2]);
...
stock UpdatePlayerVehicleParkPosition(playerid, playervehicleid, Float:newx, Float:newy, Float:newz, Float:newangle, Float:health, VW, Int)
{
	if(PlayerVehicleInfo[playerid][playervehicleid][pvId] != INVALID_PLAYER_VEHICLE_ID && GetVehicleModel(PlayerVehicleInfo[playerid][playervehicleid][pvId]))
	{
		...
		// Save current vehicle colors before destroying
		new currentColor1, currentColor2;
		GetVehicleColor(PlayerVehicleInfo[playerid][playervehicleid][pvId], currentColor1, currentColor2);
		PlayerVehicleInfo[playerid][playervehicleid][pvColor1] = currentColor1;
		PlayerVehicleInfo[playerid][playervehicleid][pvColor2] = currentColor2;
		...
		// Ensure colors are properly applied
		ChangeVehicleColours(carcreated, PlayerVehicleInfo[playerid][playervehicleid][pvColor1], PlayerVehicleInfo[playerid][playervehicleid][pvColor2]);
...
stock UpdatePlayerVehicleMods(playerid, playervehicleid)
{
	if(GetVehicleModel(PlayerVehicleInfo[playerid][playervehicleid][pvId]) && PlayerVehicleInfo[playerid][playervehicleid][pvImpounded] == 0 && PlayerVehicleInfo[playerid][playervehicleid][pvSpawned] == 1 && !PlayerVehicleInfo[playerid][playervehicleid][pvDisabled]) {
		...
		// Save current vehicle colors
		new currentColor1, currentColor2;
		GetVehicleColor(carid, currentColor1, currentColor2);
		PlayerVehicleInfo[playerid][playervehicleid][pvColor1] = currentColor1;
		PlayerVehicleInfo[playerid][playervehicleid][pvColor2] = currentColor2;
...
stock LoadPlayerVehicleMods(playerid, playervehicleid)
{
	if(GetVehicleModel(PlayerVehicleInfo[playerid][playervehicleid][pvId]) && PlayerVehicleInfo[playerid][playervehicleid][pvImpounded] == 0 && PlayerVehicleInfo[playerid][playervehicleid][pvSpawned] == 1) {
		...
		// Apply paintjob first if exists (this will change vehicle colors according to paint job)
		...
		// Apply original colors only if no paint job (paint job colors take priority)
		else
		{
...
stock CreatePlayerVehicle(playerid, playervehicleid, modelid, Float: x, Float: y, Float: z, Float: angle, color1, color2, price, VW, Int)
{
	if(PlayerVehicleInfo[playerid][playervehicleid][pvId] == INVALID_PLAYER_VEHICLE_ID)
	{
		...
		PlayerVehicleInfo[playerid][playervehicleid][pvEngineUpgrade] = 0;
		new string[256];
        format(string, sizeof(string), "INSERT INTO `vehicles` (`sqlID`, `pvEngineUpgrade`) VALUES ('%d', '0')", GetPlayerSQLId(playerid));
...
{
	// Loại bỏ chức năng vô hiệu hóa xe dựa trên VIP rank
	// Tất cả xe của người chơi sẽ luôn được kích hoạt
	for(new v = 0; v < MAX_PLAYERVEHICLES; v++)
	{
		PlayerVehicleInfo[playerid][v][pvDisabled] = 0; // Luôn kích hoạt
	}
	return 1;
}	
...
stock IsEmailPending(playerid, SQLid, const email[])
{
	new query[128];
...
stock IsPlayerInRangeOfVehicleCapo(playerid, vehicleid, Float:range)
{
    new Float:px, Float:py, Float:pz, Float:vx, Float:vy, Float:vz;
    GetPlayerPos(playerid, px, py, pz);
    GetVehiclePos(vehicleid, vx, vy, vz);
    return (GetDistanceBetweenCoords(px, py, pz, vx, vy, vz) < range);
}
