...
{
	// Optimized: Minimal processing in OnPlayerUpdate to prevent long callbacks
	static lastUpdate[MAX_PLAYERS];
	static currentTime;
	currentTime = gettime();
	
	// Throttle updates to reduce frequency
	if(currentTime - lastUpdate[playerid] < 1) return 1;
	lastUpdate[playerid] = currentTime;
	
	if(playerTabbed[playerid] >= 1)
	{
		playerTabbed[playerid] = 0;
	}
	playerSeconds[playerid] = currentTime;
...
{
	if(playertextid == Login_Panel_PTD[playerid][3]) 
    {
        new titlestring[126], string[126];
        if(GetPVarInt(playerid,"TypeShowRegister") == 1) 
        {
            SetPVarString(playerid, "GetAccountName", GetPlayerNameExt(playerid));
            format(titlestring, sizeof(titlestring), "{6084BD}Login{9E9E9E} - Character: {6084BD}%s{9E9E9E}", GetPlayerNameExt(playerid));
            format(string, sizeof(string), "{9E9E9E}Tai khoan {FFDE85}%s{9E9E9E} da duoc {6084BD}dang ky{9E9E9E}\nVui long nhap mat khau de tiep tuc.", GetPlayerNameExt(playerid));
            ShowPlayerDialog(playerid,MAINMENU,DIALOG_STYLE_PASSWORD,titlestring,string,"Xac nhan","<");
            CancelSelectTextDraw(playerid);
        }
        else if(GetPVarInt(playerid,"TypeShowRegister") == 2) 
        {
            SetPVarString(playerid, "GetAccountName", GetPlayerNameExt(playerid));
            format(titlestring, sizeof(titlestring), "{873A35}Register{9E9E9E} - Character: {873A35}%s{9E9E9E}", GetPlayerNameExt(playerid));
            format(string, sizeof(string), "{9E9E9E}Tai khoan {FFDE85}%s{9E9E9E} chua duoc {873A35}dang ky{9E9E9E}\nVui long nhap mat khau de tiep tuc.", GetPlayerNameExt(playerid));
            ShowPlayerDialog(playerid,MAINMENU2,DIALOG_STYLE_PASSWORD,titlestring,string,"Xac nhan","<");
            CancelSelectTextDraw(playerid);
        }
    }
	if(playertextid == Login_Panel_PTD[playerid][7]) 
    {
        if(GetPVarInt(playerid,"IsEnterPassword") != 1) return SendClientMessageEx(playerid,-1,"Vui long nhap mat khau de tiep tuc");
        new passw[129];
        GetPVarString(playerid,"PassAuth",passw,sizeof(passw));
        g_mysql_CreateAccount(playerid, passw);
        HideLoginPanel(playerid);
        CancelSelectTextDraw(playerid);
    }
	if(playertextid == Login_Panel_PTD[playerid][5]) 
    {
        if(GetPVarInt(playerid,"IsEnterAccount") != 1) return SendClientMessageEx(playerid,-1,"Ban da nhap sai mat khau.");
        if(GetPVarInt(playerid,"IsEnterPassword") != 1) return SendClientMessageEx(playerid,-1,"Vui long nhap mat khau de tiep tuc");
...
public OnPlayerModelSelection(playerid, response, listid, modelid)
{
	...
	if(listid == CarList2 || listid == CarList)
	{
	    if(response)
	    {
			...
			{	
				if(!vehicleCountCheck(playerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Ban da dat den gioi han toi da xe co the so huu (500 xe).");
			...
			if((InsidePlane[playerid] == GetPlayerVehicleID(i) && GetPlayerState(i) == 2) || (InsidePlane[i] == GetPlayerVehicleID(playerid) && GetPlayerState(playerid) == 2) || (InsidePlane[playerid] != INVALID_VEHICLE_ID && InsidePlane[playerid] == InsidePlane[i])) {
				...
				if(IsPlayerInRangeOfPoint(i, 20.0 / 16, f_playerPos[0], f_playerPos[1], f_playerPos[2])) {
				    ...
				    if(IsPlayerInRangeOfPoint(i, 20.0 * 0.6, f_playerPos[0], f_playerPos[1], f_playerPos[2]) && PlayerInfo[i][pBugged] >= 0 && PlayerInfo[playerid][pAdmin] < 2 && PlayerInfo[i][pAdmin] < 2)
					{
					    if(playerid == i)
					    {
							format(string, sizeof(string), "{FF9900}(Trong xe){FFFFFF} %s%s noi: %s", accent, sendername, text);
					    	format(str, sizeof(str), "{8D8DFF}(BUGGED) {CBCCCE}%s", string);
					    }
					    else {
							format(string, sizeof(string), "%s%s noi: %s", accent, sendername, text);
