public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_SHOPDELIVERCAR2)
	{
	    if(response)
	    {
			...
			{
				ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "Error", "Ban da dat den gioi han toi da xe co the so huu (500 xe)!", "Dong y", "");
			}
	...
	if(dialogid == MAINMENU || dialogid == MAINMENU2)
    {
        if(dialogid == MAINMENU)
        {
            ...
            {
				SetPVarInt(playerid, "IsEnterPassword", 1);
				SetPVarString(playerid, "PassAuth", inputtext);
				g_mysql_AccountLoginCheck(playerid);

				new string_login[65];
				new len = strlen(inputtext);
				for(new i = 0; i < len; i++) string_login[i] = '.';
				string_login[len] = '\0';

				PlayerTextDrawSetString(playerid, Login_Panel_PTD[playerid][3], string_login);
        ...
        else if(dialogid == MAINMENU2)
        {
            if(!isnull(inputtext) && strlen(inputtext) < 64)
            {
                ...
                PlayerTextDrawSetString(playerid, Login_Panel_PTD[playerid][3], string);
                SelectTextDraw(playerid, COLOR_YELLOW);
            }
        }
	...
	if(dialogid == VEHICLESTORAGE && response) {
		...
		// Loại bỏ tùy chọn mua slot - người chơi có thể add xe trực tiếp
		...
		// Loại bỏ kiểm tra xe bị vô hiệu hóa
	            ...
	            case 1:
	            {
					...
					for(new i, iModelID; i < MAX_PLAYERVEHICLES; i++) {
						if((iModelID = PlayerVehicleInfo[playerid][i][pvModelId] - 400) >= 0) {
							...
							// Loại bỏ hiển thị xe bị vô hiệu hóa
...
/*	if(dialogid == MDC_CHECK && response)
	...
	else if(dialogid == DIALOG_CDBUY)
	{
		...
		{
			if(!vehicleCountCheck(playerid))
				return SendClientMessageEx(playerid, COLOR_GREY, "ERROR: Ban da dat den gioi han toi da xe co the so huu (500 xe).");
	...
	if(dialogid == DIALOG_CARSHOP)
	{
	    if(response)
	    {
	    	if(PlayerInfo[playerid][pCredits] < ShopItems[5][sItemPrice])
				...
				return ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "Error", "Ban da dat den gioi han toi da xe co the so huu (500 xe)!", "Dong y", "");
	...
	if(dialogid == DIALOG_CARSHOP2)
	{
	    if(response)
	    {
	    	if(PlayerInfo[playerid][pCarVoucher] == 0)
				...
				return ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "Loi", "Ban da dat den gioi han toi da xe co the so huu (500 xe)!", "Dong y", "");
